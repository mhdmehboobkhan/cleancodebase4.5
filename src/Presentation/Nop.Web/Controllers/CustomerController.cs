...
namespace Nop.Web.Controllers
{
    ...
    public partial class CustomerController : BasePublicController
    {
        ...
        public virtual async Task<IActionResult> Addresses()
        {
            ...
            var model = await _customerModelFactory.PrepareCustomerAddressSearchModelAsync(new CustomerAddressSearchModel());
        ...
        public virtual async Task<IActionResult> AddressesSelect(CustomerAddressSearchModel searchModel)
        {
            ...
            //prepare model
            var model = await _customerModelFactory.PrepareCustomerAddressListModelAsync(searchModel);
        ...
        public virtual async Task<IActionResult> Notes()
        {
            ...
            //prepare model
            var model = _customerModelFactory.PrepareCustomerNoteSearchModel(new CustomerNoteSearchModel());

            return View(model);
        }

        [HttpPost]
        public virtual async Task<IActionResult> NotesSelect(CustomerNoteSearchModel searchModel)
        {
            var customer = await _workContext.GetCurrentCustomerAsync();
            if (!await _customerService.IsRegisteredAsync(customer))
                return await AccessDeniedDataTablesJson();
            ...
            //prepare model
            var model = await _customerModelFactory.PrepareCustomerNoteListModelAsync(searchModel);

            return Json(model);
        }

        public virtual async Task<IActionResult> NoteAdd(string message, int downloadId = 0)
        {
            var customer = await _workContext.GetCurrentCustomerAsync();
            if (!await _customerService.IsRegisteredAsync(customer))
            ...
            if (string.IsNullOrEmpty(message))
                return ErrorJson(await _localizationService.GetResourceAsync("Admin.Customers.CustomerNotes.Fields.Note.Validation"));
            ...
            await _customerService.InsertCustomerNoteAsync(customerNote);

            //email
            await _workflowMessageService.SendNewCustomerNoteAddedStoreOwnerNotificationAsync(customerNote, (await _workContext.GetWorkingLanguageAsync()).Id);

            return Json(new { Result = true });
        }

        [HttpPost]
        public virtual async Task<IActionResult> NoteDelete(int id)
        {
            var customer = await _workContext.GetCurrentCustomerAsync();
            if (!await _customerService.IsRegisteredAsync(customer))
                return ErrorJson("Customer cannot be loaded");

            //try to get an customer note with the specified id
            var customerNote = await _customerService.GetCustomerNoteByIdAsync(id)
                ?? throw new ArgumentException("No customer note found with the specified id");

            await _customerService.DeleteCustomerNoteAsync(customerNote);
