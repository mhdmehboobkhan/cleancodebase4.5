            ...
            if (ModelState.IsValid)
            {
                ...
                if (registrationResult.Success)
                {
                    ...
                    if (_customerSettings.NewsletterEnabled)
                    {
                        ...
                        var newsletter = await _newsLetterSubscriptionService.GetNewsLetterSubscriptionByEmailAndStoreIdAsync(model.Email,
                            (await _storeContext.GetCurrentStoreAsync()).Id);
                        if (newsletter != null)
                        {
                            if (model.Newsletter)
                    ...
                    var defaultAddress = new Address
                    {
                        ...
                        CountryId = model.CountryId > 0 ? (int?)model.CountryId : null,
                        StateProvinceId = model.StateProvinceId > 0 ? (int?)model.StateProvinceId : null,
                        County = model.County,
                        City = model.City,
