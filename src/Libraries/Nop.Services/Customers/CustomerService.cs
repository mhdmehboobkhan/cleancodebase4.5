...
namespace Nop.Services.Customers
{
    ...
    public partial class CustomerService : ICustomerService
    {
        ...
        /// Gets an order note
        /// </summary>
        /// <param name="customerNoteId">The order note identifier</param>
        /// <returns>
        /// A task that represents the asynchronous operation
        /// The task result contains the order note
        /// </returns>
        public virtual async Task<CustomerNote> GetCustomerNoteByIdAsync(int customerNoteId)
        {
            return await _customerNoteRepository.GetByIdAsync(customerNoteId);
        }
        ...
        /// The task result contains the result
        /// </returns>
        public virtual async Task<IPagedList<CustomerNote>> GetAllCustomerNotesAsync(int customerId = 0, bool? displayToCustomer = null,
            DateTime? dateFrom = null, DateTime? dateTo = null, int pageIndex = 0, int pageSize = int.MaxValue, bool getOnlyTotalCount = false)
        {
            return await _customerNoteRepository.GetAllPagedAsync(async query =>
            {
                if (customerId > 0)
                    query = query.Where(cn => cn.CustomerId == customerId);

                if (displayToCustomer.HasValue)
                    query = query.Where(cn => cn.DisplayToCustomer == displayToCustomer);

                if (dateFrom.HasValue)
                    query = query.Where(cn => dateFrom.Value <= cn.CreatedOnUtc);

                if (dateTo.HasValue)
                    query = query.Where(cn => dateTo.Value >= cn.CreatedOnUtc);

                query = query.OrderByDescending(cn => cn.CreatedOnUtc);
        ...
        /// </summary>
        /// <param name="customerNote">The order note</param>
        /// <returns>A task that represents the asynchronous operation</returns>
        public virtual async Task DeleteCustomerNoteAsync(CustomerNote customerNote)
        {
        ...
        /// </summary>
        /// <param name="customerNote">Order note</param>
        /// <returns>Formatted text</returns>
        public virtual string FormatCustomerNoteText(CustomerNote customerNote)
        {
            if (customerNote == null)
                throw new ArgumentNullException(nameof(customerNote));

            var text = customerNote.Note;
        ...
        /// <param name="customerNote">The order note</param>
        /// <returns>A task that represents the asynchronous operation</returns>
        public virtual async Task InsertCustomerNoteAsync(CustomerNote customerNote)
        {
